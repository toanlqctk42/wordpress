<?php 
add_action( 'wp_enqueue_scripts', 'image_shareify_enqueue_styles' );
function image_shareify_enqueue_styles() {
	wp_enqueue_style( 'image-shareify-parent-style', get_template_directory_uri() . '/style.css' ); 
} 

function image_shareify_customize_preview_js() {
	wp_enqueue_script( 'image-shareify-customizer', get_stylesheet_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '1', true );
}
add_action( 'customize_preview_init', 'image_shareify_customize_preview_js' );

function image_shareify_google_fonts() {
	wp_enqueue_style( 'image-shareify-google-fonts', '//fonts.googleapis.com/css2?family=Roboto:wght@400;500;700&display=swap', false ); 
}
add_action( 'wp_enqueue_scripts', 'image_shareify_google_fonts' );


require_once( get_stylesheet_directory() . '/inc/custom-header.php' );


function image_shareify_customize_register( $wp_customize ) {	
	...
	$wp_customize->add_control( 'only_show_header_frontpage', array(
		'label'    => __( 'Only show header image on front page', 'image-shareify' ),
	...
	$wp_customize->add_control( 'header_img_text_tagline', array(
		'label'    => __( "Tagline", 'image-shareify' ),
		'section'  => 'header_image',
		'type'     => 'text',
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_img_textcolor', array(
		'label'       => __( 'Text Color', 'image-shareify' ),
		'section'     => 'header_image',
		'priority'   => 1,
		'settings'    => 'header_img_textcolor',
	) ) );

   $wp_customize->add_section('post_excerpt_options', array(
    'title'      => __('Post Excerpt', 'image-shareify'),
    'priority'   => 1,
    'capability' => 'edit_theme_options',
   ...
   $wp_customize->add_control('show_post_excerpt', array(
    'label'    => __('Show post excerpt', 'image-shareify'),
    'section'  => 'post_excerpt_options',
    'priority' => 0,
...
add_action( 'customize_register', 'image_shareify_customize_register', 999 );




if(! function_exists('image_shareify_customize_register_output' ) ):
	function image_shareify_customize_register_output(){
		?>

		<style type="text/css">

		.content-wrapper .post-data-text .about-the-author-byline, .single .post-data-text .about-the-author-byline{ color: <?php echo esc_attr(get_theme_mod( 'image_shareify_abouttheauthor_color')); ?>; }
		a.read-more-btn{ background: <?php echo esc_attr(get_theme_mod( 'image_shareify_readmore_bg')); ?>; }
		a.read-more-btn{ color: <?php echo esc_attr(get_theme_mod( 'image_shareify_readmore_text')); ?>; }
...
<?php }
add_action( 'wp_head', 'image_shareify_customize_register_output' );
endif;
